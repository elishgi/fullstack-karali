...
const getMailTransport = () => {
  ...
  if (nodemailer) {
    ...
    console.warn(
      'Falling back to a console logger because Nodemailer is unavailable. Install the "nodemailer" package to enable real e-mails.'
    );
    mailTransport = {
      async sendMail(message) {
        console.info('[mail:mock] Password reset e-mail payload:', {
          to: message.to,
...
const googleAuth = async (req, res) => {
  try {
    ...
    if (!googleClient) {
      return res.status(503).json({ message: 'שירות גוגל אינו זמין כרגע בצד השרת (חסר תלות google-auth-library).' });
    }
